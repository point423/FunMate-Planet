<template>
  <div class="auth-page">
    <div class="auth-card glass-card">
      ...
      <p class="auth-sub">Pick tags that describe what you love doing. We'll use these to find your perfect partner.</p>
...
const onSave = async () => {
  loading.value = true
  try { await updateTags(tags.value); router.push('/') } finally { loading.value = false }
}
</script>

<style scoped>
.auth-page  { min-height:100vh; display:flex; align-items:center; justify-content:center; position:relative; z-index:1; }
.auth-card  { width:560px; padding:40px; display:flex; flex-direction:column; gap:24px; }
.auth-title { font-family:var(--font-display); font-size:26px; text-align:center; }
.auth-sub   { font-size:14px; color:var(--color-text-secondary); text-align:center; }
</style>
